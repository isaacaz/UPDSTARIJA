...
export class HomePage {
  ...
  constructor(
    ...
    platform.ready().then(() => {
      if (typeof FCMPlugin !== "undefined") {
        ...
        console.log(
          "Notifications disabled, only provided in Android/iOS environment"
        );
    });
    if (this.platform.is("android")) {
      this.db
        .getRealTimeDBObject("upds/update/android")
        .subscribe((upd: any) => {
          if (upd.version) {
  ...
  openExternalWeb(url) {
    ...
    if (url != "" || url != null) {
      this.browser.isAvailable().then((isAvailable: boolean) => {
        if (isAvailable) {
  ...
  getBanner() {
    ...
    //1
    this.db
      .getRealTimeDBQuery("upds/banner/1", (ref) =>
    ...
    //2
    this.db
      .getRealTimeDBQuery("upds/banner/2", (ref) =>
        ref.orderByChild("timestamp").limitToLast(1)
      )
      .subscribe((data: any) => {
    ...
    //3
    this.db
      .getRealTimeDBQuery("upds/banner/3", (ref) =>
        ref.orderByChild("timestamp").limitToLast(1)
      )
      .subscribe((data: any) => {
    ...
    //4
    this.db
      .getRealTimeDBQuery("upds/banner/4", (ref) =>
        ref.orderByChild("timestamp").limitToLast(1)
      )
      .subscribe((data: any) => {
    ...
    //5
    this.db
      .getRealTimeDBQuery("upds/banner/5", (ref) =>
        ref.orderByChild("timestamp").limitToLast(1)
      )
      .subscribe((data: any) => {

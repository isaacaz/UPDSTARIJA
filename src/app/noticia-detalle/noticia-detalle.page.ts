...
export class NoticiaDetallePage implements OnInit {
  ...
  getNoticia() {
    ...
    this.db
      .getRealTimeDBObject("upds/noticias/" + this.key)
      .subscribe((noticia) => {
        this.noticia = noticia;
      });

    this.db
      .getRealTimeDBList("upds/noticias/" + this.key + "/images")
      .subscribe((imgs) => {
        this.images = imgs;
        imgs.map((img: any) => {});
      });
    ...
    this.storage.get(key).then((val) => {
      if (val) {
        this.storage.remove(key).then(() => {
          this.db
            .getRealTimeDBObjectPromise("upds/noticias/" + key)
            .then((o: any) => {
              o.likes -= 1;
  ...
  share(message, imgUrl) {
    if (imgUrl) {
      this.convertToDataURLviaCanvas(imgUrl, "image/png").then(
        (base64Img: string) => {
          this.social
            .share(
              this.noticia.title +

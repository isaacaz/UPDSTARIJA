...
export class ConsultaPage implements OnInit {
  ...
  toast:any;
  lista:any;
  constructor(public loadingCtrl: LoadingController, private db: DatabaseService, public toastCtrl: ToastController, private alert: AlertController, public modalcontroller: ModalController, private storage: Storage, private device:Device) { }
  ...
  ngOnInit() {
    ...
    ref=>ref.orderByChild('uuid').equalTo(this.device.uuid).limitToLast(1))
      .subscribe(res => {
      if (res[0]) {
        this.consult = {
      ...
      buttons: [{
        ...
        handler:async () => {
          ...
          FCMPlugin.getToken((t) => {
            console.log(datos);
            datos.fcm = t;
            this.db.pushRealTimeObject('upds/consultas',datos).then(res=>{
              loading.dismiss();
              //this.presentLoading();
              this.presentToast();
